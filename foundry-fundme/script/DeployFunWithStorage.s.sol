...
contract DeployFunWithStorage is Script {
    ...
    function printFirstArrayElement(address contractAddress) public view {
        bytes32 arrayStorageSlotLength = bytes32(uint256(2)); // 2 is the SLOT of the array
    ...
    function printSecondArrayElement(address contractAddress) public view {
        bytes32 arrayStorageSlotLength = bytes32(uint256(2)); // 2 is the SLOT of the array
        bytes32 secondElementStorageSlot = keccak256(
            abi.encode(arrayStorageSlotLength)
        );

        uint secondElementSlot = uint(secondElementStorageSlot) + 1;
        bytes32 value = vm.load(contractAddress, bytes32(secondElementSlot));
